...
//Removed GraphQLClient import as it's not used with current mock data approach.
...
// const SEARCH_NFTS_QUERY = gql`...`; // Keep if you plan to use GraphQL later
...
export default function SearchPage() {
  ...
  const [isLoadingResults, setIsLoadingResults] = useState(false); // Set to false initially
  ...
  const fetchAndSimulate = useCallback(async (query: string, category: string, priceRange: string, statusFilterValue: string, sort: SortOption) => {
    ...
    try {
      ...
      const statusMap: Record<string, EnhancedNFTCardProps['status'][]> = {
        'buy-now': ['listed'], // 'listed' can be bought now if not an auction
        'on-auction': ['on_auction'],
        'all-status': ['listed', 'on_auction', 'sold', 'hidden', 'draft', 'pending_moderation'] // Or whatever is searchable
      };
  ...
  const statusOptions = [
      ...
      { value: 'buy-now', label: 'Buy Now (Listed)' }, // Clarified label
            ...
            viewMode === 'grid' ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                {itemsToRender.map(nft => (
                  // Ensure priceEth is passed to NFTCard for internal use if needed, though NFTCard expects `price` as string
