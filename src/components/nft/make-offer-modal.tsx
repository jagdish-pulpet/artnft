...
import type { Nft } from "@/types/entities"; // Import Nft type
...
interface MakeOfferModalProps {
  ...
  nft: Nft; 
  onOfferMade: () => void; // Callback to refresh offers list on NFT detail page
}
...
export function MakeOfferModal({ isOpen, onOpenChange, nft, onOfferMade }: MakeOfferModalProps) {
  ...
  const form = useForm<MakeOfferFormValues>({
    ...
    defaultValues: {
      currency: nft?.currency || "ETH", // Default to NFT's currency or ETH
  ...
  const onSubmit = async (data: MakeOfferFormValues) => {
    ...
    try {
      ...
      toast({ title: "Offer Submitted", description: `Your offer for ${nft.title} has been submitted.` });
      onOfferMade(); 
      onOpenChange(false); 
    } catch (error: any) {
      const errorMessage = error instanceof ApiError ? error.data?.message || error.message : "Failed to submit offer.";
    ...
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Enter your offer details below. All offers are binding if accepted by the owner.
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
             ...
             <FormField
              ...
              render={({ field }) => (
                <FormItem className="flex flex-col">
                  ...
                  <Popover>
                    ...
                    <PopoverContent className="w-auto p-0" align="start">
                      <Calendar
                        ...
                        initialFocus
                      />
                      {/* Basic Time Picker - replace with a proper time picker if needed */}
                      <div className="p-3 border-t border-border">
                        ...
                        <Input type="time" 
                            ...
                            onChange={(e) => {
                                ...
                                } else if (time) { // if date not picked yet, set a default date (today)
                                    const [hours, minutes] = time.split(':').map(Number);
                  ...
                  <FormDescription>
                    When your offer will automatically expire.
                  </FormDescription>
                  <FormMessage />
                </FormItem>
              )}
