...
import { ArrowLeft, ShoppingCart, Tag, Info, ListFilter, ThumbsUp, Eye, Star, Share2, MoreHorizontal, AlertCircle, Activity as ActivityIconLucide, Loader2, HandCoins, ListChecks, Wallet, CalendarClock, X, Check, Trash2 } from 'lucide-react';
import {
  Accordion,
  AccordionContent,
  AccordionItem,
...
import type { Nft as NftDetailData, NftActivityItem, Offer, OfferStatus, PaginatedResponse } from '@/types/entities';
...
export default function NftDetailPage() {
  ...
  const fetchNftDetails = useCallback(async () => {
    ...
    try {
      ...
      // In a real app, you'd fetch the user's favorite status for this NFT
      // For now, let's assume it's not favorited by default.
      // This could be part of the /nfts/:id response if the user is authenticated.
    } catch (err: any) {
  ...
  const fetchNftActivity = useCallback(async () => {
    ...
    try {
      const activityResponse = await apiService.get<PaginatedResponse<NftActivityItem>>(`/nfts/${id}/activity?limit=10&page=1`);
  ...
  const fetchNftOffers = useCallback(async () => {
    ...
    try {
      const response = await apiService.get<PaginatedResponse<Offer>>(`/nfts/${id}/offers?limit=50`);
  ...
  const handleOfferAction = async () => {
    ...
    let endpoint = '';
    let method: 'put' | 'del' = 'put'; // For API service
    let successMessage = '';

    switch(confirmActionType) {
        case 'accept': 
            endpoint = `/offers/${actionOfferId}/accept`; 
            successMessage = "Offer accepted successfully! Ownership will be updated shortly.";
            break;
        case 'reject': 
        ...
        case 'cancel': 
            ...
            method = 'del'; // DELETE request for cancel
    ...
    try {
        if (method === 'del') {
            await apiService.del(endpoint, token);
        } else {
            await apiService.put(endpoint, {}, token); // Empty body for PUT
        }
        toast({ title: "Success", description: successMessage });
        refreshData(); // Re-fetch NFT details, offers, and activity
    } catch (error: any) {
  ...
  // Assuming a fixed ETH to USD rate for display simplicity
  const ethToUsdRate = 3000; 
  const priceUsd = nftData?.price && (nftData.currency === 'ETH' || nftData.currency === 'WETH') ? (nftData.price * ethToUsdRate).toLocaleString('en-US', { style: 'currency', currency: 'USD' }) : null;
    ...
    <>
      ...
      <main className="min-h-screen bg-gradient-to-br from-background via-background to-primary/5 p-4 sm:p-6 md:p-8 selection:bg-accent/30 selection:text-accent-foreground">
          ...
          <div className="mb-6">
            <Button variant="outline" asChild>
              <Link href={nftData.collectionId ? `/collections/${nftData.collection?.slug || nftData.collectionId}` : "/collections"}>
                ...
                Back to {nftData.collection?.name || "Collections"}
              </Link>
            </Button>
          </div>
            ...
            <div className="lg:col-span-3 space-y-6">
              <Card className="shadow-xl rounded-xl">
                <CardHeader>
                  <div className="flex justify-between items-start">
                    <div>
                      {nftData.collection?.name && (<Link href={nftData.collectionId ? `/collections/${nftData.collection.slug || nftData.collectionId}` : `/collections?search=${encodeURIComponent(nftData.collection.name)}`} className="hover:underline"><CardDescription className="text-accent font-semibold text-sm mb-1">{nftData.collection.name}</CardDescription></Link>)}
                  ...
                  <div className="text-sm text-muted-foreground pt-1">Owned by <Link href={`/profile/${nftData.owner.id}`} className="text-accent hover:underline font-medium">{nftData.owner.username}</Link> {nftData.owner.walletAddress && `(${nftData.owner.walletAddress.substring(0,6)}...${nftData.owner.walletAddress.slice(-4)})`}</div>
                  <div className="flex items-center space-x-4 text-sm text-muted-foreground pt-2">
                    <span className="flex items-center"><Eye className="mr-1.5 h-4 w-4"/> {nftData.views?.toLocaleString() || 0} views</span>
                ...
                <CardContent className="space-y-6">
                  ...
                  <div className="flex flex-col sm:flex-row gap-3 w-full">
                    {nftData.isListedForSale && nftData.price !== null && nftData.price !== undefined && !isOwner && (
                      <Button size="lg" className="flex-1 font-semibold py-3 text-base" onClick={() => toast({title: "Buy Now (Placeholder)", description: "This feature is coming soon!"})}><ShoppingCart className="mr-2 h-5 w-5" /> Buy Now</Button>
                    )}
                    {canMakeOffer && (
                    ...
                    {isOwner && nftData.isListedForSale && (
                         <Button size="lg" variant="outline" className="flex-1 font-semibold py-3 text-base" onClick={() => toast({title: "Manage Listing (Placeholder)"})}>Manage Listing</Button>
                    )}
                     {isOwner && !nftData.isListedForSale && (
                         <Button size="lg" variant="default" className="flex-1 font-semibold py-3 text-base" onClick={() => toast({title: "List for Sale (Placeholder)"})}>List for Sale</Button>
                    )}
                  </div>
                </CardContent>
              </Card>
              ...
              <Accordion type="multiple" defaultValue={["description", "properties", "offers"]} className="w-full space-y-4">
                ...
                <AccordionItem value="offers" className="bg-card rounded-xl shadow-xl px-4 sm:px-6 border-border/50">
                    <AccordionTrigger className="text-lg font-semibold hover:no-underline text-primary py-4"><ListChecks className="mr-2 h-5 w-5" /> Offers ({nftOffers.filter(o => o.status === OfferStatus.PENDING).length})</AccordionTrigger>
                        ...
                        <ul className="space-y-3">
                            ...
                            <li key={offer.id} className="flex flex-col sm:flex-row justify-between items-start p-3 bg-muted/30 rounded-md shadow-sm gap-2 sm:gap-4">
                                <div className="flex-1">
                                    ...
                                    {offer.expiresAt && <p className="text-xs text-muted-foreground mt-0.5 flex items-center"><CalendarClock className="h-3 w-3 mr-1"/>Expires {formatDistanceToNowStrict(new Date(offer.expiresAt), { addSuffix: true })}</p>}
                                </div>
                                <div className="flex gap-2 items-center self-start sm:self-center mt-2 sm:mt-0">
                                    ...
                                    {authUser?.id === offer.offererId && (<Button size="xs" variant="ghost" className="text-muted-foreground hover:text-destructive" onClick={() => openConfirmation(offer.id, 'cancel')} disabled={isProcessingAction}>{isProcessingAction && actionOfferId === offer.id && confirmActionType === 'cancel' ? <Loader2 className="h-4 w-4 animate-spin"/> : <Trash2 className="h-4 w-4 mr-1"/>}Cancel</Button>)}
                                </div>
                            </li>
                            ))}
                        </ul>
        ...
        <ConfirmationDialog 
            ...
            title={`Confirm Offer ${confirmActionType ? confirmActionType.charAt(0).toUpperCase() + confirmActionType.slice(1) : ''}`}
